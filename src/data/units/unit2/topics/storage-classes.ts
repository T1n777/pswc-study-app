...
export const storageClasses: Topic = {
  ...
  description: 'Every variable in your C program exists somewhere in memory, lasts for some duration, starts with some initial value, and is visible from some set of locations in your code. Storage classes are the mechanism through which you control all four of these properties. The auto keyword (which is the default and rarely written explicitly) creates variables on the stack that are born when their block starts and die when it ends. The static keyword extends a local variable\'s lifetime to the entire program while keeping its name private to the function. The extern keyword lets you access a variable defined in a different source file. And register (mostly obsolete) hints that a variable should live in a CPU register for speed. Understanding storage classes means understanding the memory layout of your program: the stack, the data segment, the BSS segment, and CPU registers.',
  ...
  subtopics: [
    {
      ...
      slug: 'local-storage',
      description: 'Every local variable has three properties you should understand: where it is stored (the stack by default), how long it lives (until the enclosing block ends by default), and what its initial value is (garbage by default). The auto, register, and static keywords let you change these defaults. The most important of these is static, which moves a local variable off the stack and into the data segment, giving it a lifetime that spans the entire program execution while keeping its name visible only within the function. This creates a powerful pattern: a variable that remembers its value between function calls but is invisible to all other code.',
      keyPoints: [
    ...
    },
    {
      ...
      slug: 'global-storage',
      description: 'When a C program grows beyond a single file, you need mechanisms to share data between files and to protect private data from being accessed by other files. The extern keyword provides sharing: it tells the compiler that a variable exists but is defined in another source file, deferring resolution to the linker. The static keyword applied to a global variable provides protection: it restricts the variable\'s visibility to the file where it is declared. These two keywords together form C\'s module system — a crude but effective way to manage encapsulation in large programs.',
      keyPoints: [
