...
export const recursion: Topic = {
  ...
  title: 'Recursion',
  slug: 'recursion',
  description: 'Recursion is one of the most intellectually challenging concepts in programming, but once it clicks, it becomes one of the most elegant. A recursive function is one that calls itself, breaking a large problem into smaller instances of the same problem until reaching a trivially simple case. The classic example is factorial: 5! = 5 * 4!, and 4! = 4 * 3!, and so on until 1! = 1 (the trivial case). But recursion is not just a mathematical curiosity — it is the natural solution for inherently hierarchical problems like traversing trees, exploring graphs, parsing nested expressions, and implementing divide-and-conquer algorithms. Understanding recursion also deepens your understanding of the call stack, because every recursive call adds a new frame, and the "magic" of recursion happens when those frames unwind.',
  difficulty: 'advanced',
  prerequisites: ['u2-t6'],
  estimatedMinutes: 90,
  subtopics: [
    {
      ...
      description: 'Every recursive function must have two essential components: a base case that stops the recursion, and a recursive case that breaks the problem into a smaller version of itself. The base case is what prevents infinite recursion — without it, the function calls itself forever until the call stack overflows and the program crashes. The recursive case must make progress toward the base case with every call, typically by reducing a number, shortening a string, or narrowing a search range. The art of writing a recursive function is identifying these two cases: what is the simplest version of the problem (base case), and how can I express the general case in terms of a slightly simpler version (recursive case)?',
      keyPoints: [
    ...
    },
    {
      ...
      description: 'The key to understanding recursion is visualizing the call stack during execution. Every recursive call creates a new stack frame that waits for the deeper call to return before it can complete its own computation. This creates two distinct phases: the winding phase (where calls go deeper and deeper, building up stack frames) and the unwinding phase (where the base case is hit and results propagate back up as each frame completes its computation and returns). Tracing through these phases on paper is the single most effective technique for understanding any recursive function.',
      keyPoints: [
