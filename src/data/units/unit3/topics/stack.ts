  ...
  subtopics: [
    {
      ...
      slug: 'stack-lifo',
      description: 'A Stack is fundamentally an Abstract Data Type (ADT). This means "Stack" is not a built-in C feature like `int` or `struct`. Rather, it is a behavioral contract. You construct it using an Array or a Linked List, but you artificially restrict access so the programmer can ONLY interact with the topmost element.',
      keyPoints: [
    ...
    },
    {
      ...
      description: 'An Array-based Stack is crippled by its hard-coded `MAX` capacity. By substituting the array with a Linked List, the Stack can dynamically scale to consume all available physical RAM. Architecturally, we map the Stack "Top" directly to the Linked List "Head".',
      keyPoints: [
