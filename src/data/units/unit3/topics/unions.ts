  ...
  subtopics: [
    {
      ...
      description: 'The defining characteristic of a union is complete memory overlap. If a structure has an `int` and a `double`, the compiler allocates 4 bytes, leaves some padding, and then allocates 8 bytes, totaling 12+ bytes. If a union has an `int` and a `double`, the compiler allocates exactly 8 bytes (the size of the largest member), and both the `int` and the `double` start at byte 0. Because they overlap, you can only safely store one piece of data at any given time.',
      keyPoints: [
    ...
    },
    {
      ...
      slug: 'tagged-unions',
      description: 'A raw union is inherently dangerous because it lacks self-awareness: there is no way to ask a union whether it currently holds an int, a float, or a string. If you guess incorrectly and read the wrong member, your program processes garbage data without crashing. To fix this, professional C code almost always wraps unions inside a struct alongside an integer or `enum`. This integer acts as a "tag" that definitively tracks which union member is currently active.',
      keyPoints: [
