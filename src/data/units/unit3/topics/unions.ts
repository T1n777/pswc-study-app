...
export const unions: Topic = {
  ...
  title: 'Unions',
  slug: 'unions',
  description: 'While structures group different variables into separate blocks of memory, a union takes a radically different approach: it forces all of its members to share the exact same physical memory address. Think of a union as a single storage container with multiple different labels on it, each interpreting the contents differently. You can write an integer into the container, and then read it out as a float, allowing you to peek directly into how the CPU represents data at a binary level. While unions are historically used to save memory in highly constrained embedded systems, their primary modern use is in "type punning" and building polymorphic variables that can change their data type dynamically.',
  difficulty: 'advanced',
  prerequisites: ['u3-t5'],
  estimatedMinutes: 40,
  subtopics: [
