...
export const salientFeatures: Topic = {
  ...
  description:
    'Before you write your first line of C, it helps to understand where this language came from and why, after more than fifty years, it remains one of the most important programming languages in the world. C was not designed in a vacuum — it was born out of a specific need (rewriting the UNIX operating system) and shaped by specific constraints (limited memory, slow processors, the need for portability). Every design decision in C — from its minimalism to its trust in the programmer — reflects those origins. Understanding C\'s history and characteristics gives you insight into why it behaves the way it does, even when that behavior seems strange compared to modern languages.',
  difficulty: 'beginner',
  prerequisites: ['u1-t1'],
  estimatedMinutes: 30,
  ...
  subtopics: [
    {
      ...
      slug: 'history-of-c',
      description:
        'The story of C begins in the late 1960s at Bell Labs (AT&T\'s research division), where Ken Thompson and Dennis Ritchie were building the UNIX operating system. The first version of UNIX was written in assembly language, which was fast but nightmarishly difficult to maintain and impossible to port to different hardware. Thompson created a simpler language called B (derived from BCPL), but B lacked data types and was still too primitive for serious systems work. In 1972, Ritchie extended B into a new language he called C, adding the type system, structures, and the ability to generate efficient machine code. The result was a language powerful enough to write an operating system but portable enough to move that operating system to new hardware with relatively little effort.',
      keyPoints: [
    ...
    },
    {
      ...
      description:
        'Every programming language makes trade-offs, and understanding C\'s specific trade-offs helps you appreciate both its power and its pitfalls. C prioritizes speed, predictability, and hardware access over safety, convenience, and ease of learning. It is structured (encouraging modular code through functions), statically typed (catching type errors at compile time), portable (running on virtually any platform), fast (compiling to native machine code), close to the hardware (giving you direct memory access), and deliberately small (with fewer keywords than nearly any other mainstream language). These characteristics are not accidental — they are the direct result of C being designed to write operating systems on machines with very limited resources.',
      keyPoints: [
