...
export const variables: Topic = {
  ...
  slug: 'variables',
  description:
    'A variable is your program\'s way of remembering things. When you write int age = 19, you are asking the computer to set aside a small piece of its memory, label it "age," and store the number 19 there so you can use it later. But unlike variables in mathematics, where x is an abstract symbol, a variable in C is a real, physical location in the computer\'s RAM with a concrete address, a fixed size, and a value that can change as your program runs. Understanding this physical reality is key to understanding why C behaves the way it does.',
  difficulty: 'beginner',
  prerequisites: ['u1-t3'],
  estimatedMinutes: 35,
  ...
  subtopics: [
    {
      ...
      description:
        'When you declare a variable in C, something very concrete happens inside the computer: the compiler allocates a specific number of bytes in RAM, assigns that block of memory an address (like a house number on a street), and associates your chosen name with that address so you never have to think in raw numbers. Every time you write "age" in your code, the compiler secretly translates it into "go to memory address 0x7ffd2a3c and read the 4 bytes stored there." This mapping from human-friendly names to machine addresses is one of the most important services a programming language provides.',
      keyPoints: [
    ...
    },
    {
      ...
      description:
        'Choosing a good name for a variable is more important than most beginners realize. Your code will be read far more often than it is written — by your future self, by your teammates, by your viva examiner. A variable named x tells the reader nothing, while student_count tells them exactly what it represents. C enforces certain naming rules (what characters are legal, what words are reserved) and the programming community has established conventions (snake_case, UPPER_CASE for constants) that make code instantly readable to any C programmer.',
      keyPoints: [
