...
export const codingStandards: Topic = {
  ...
  description: 'Writing code that works is only half the job — writing code that other people can read, understand, and maintain is the other half. In professional software development, you spend far more time reading code than writing it, so the readability of your code has a direct impact on productivity, bug rates, and team collaboration. This topic covers the naming conventions, formatting rules, commenting practices, and compiler flag habits that distinguish professional-quality C code from student-quality code. These are not arbitrary aesthetic preferences — they are engineering practices proven by decades of industry experience.',
  difficulty: 'beginner',
  prerequisites: ['u1-t3'],
  estimatedMinutes: 25,
  subtopics: [
    {
      ...
      description: 'The three pillars of readable code are descriptive names, consistent formatting, and purposeful comments. Naming is the most important: a well-chosen variable or function name can eliminate the need for a comment entirely. Indentation creates visual structure that mirrors logical structure, making control flow instantly visible. And comments, when used correctly, explain the why behind code rather than restating the what. Together, these practices transform code from a puzzle to be decoded into a document to be read.',
      keyPoints: [
    ...
    },
    {
      ...
      description: 'The compiler is your first and most tireless code reviewer. When you enable GCC\'s warning flags, you transform it from a mere translator ("convert my C to machine code") into a vigilant mentor that catches bugs before they become runtime problems. The -Wall flag alone can detect unused variables, missing return statements, format specifier mismatches, and dozens of other issues that would otherwise become silent bugs. Making warning-free compilation a personal standard is one of the highest-leverage habits you can develop as a programmer.',
      keyPoints: [
