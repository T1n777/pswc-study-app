...
export const programStructure: Topic = {
  ...
  description:
    'Every C program, no matter how complex, follows the same fundamental blueprint: preprocessor directives at the top, function definitions in the middle, and a main() function that serves as the starting gate. Understanding this structure is not just about memorizing syntax — it is about understanding the journey your code takes from human-readable text to a running program, passing through four distinct stages of transformation along the way.',
  difficulty: 'beginner',
  prerequisites: ['u1-t2'],
  estimatedMinutes: 40,
  ...
  subtopics: [
    {
      ...
      description:
        'If you open any C program in the world, from a student\'s first homework to the Linux kernel, you will find the same structural elements: preprocessor directives that prepare your code before compilation, function definitions that organize your logic into reusable blocks, and the special main() function that the operating system calls to start your program. Understanding why each piece exists — not just that it exists — is what transforms syntax memorization into genuine comprehension.',
      keyPoints: [
    ...
    },
    {
      ...
      description:
        'When you type gcc hello.c -o hello and press Enter, four separate programs run in sequence, each transforming your code into a lower-level representation until it becomes something the CPU can execute directly. Understanding this pipeline is not academic trivia — it is practical knowledge that helps you diagnose build errors. A preprocessor error (like a missing header file) looks very different from a compiler error (like a syntax mistake), which looks different from a linker error (like a missing library). Knowing which stage produced the error tells you where to look for the fix.',
      keyPoints: [
